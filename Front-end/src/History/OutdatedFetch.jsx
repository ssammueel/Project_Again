...
export const Outdated = () => {
  ...
  const fetchScans = async () => {
    ...
    try {
      ...
      if (searchTarget) url += `&target=${encodeURIComponent(searchTarget)}`;
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `outdated-scans-${new Date().toISOString()}.csv`);
  ...
  const exportToJSON = () => {
    ...
    link.setAttribute("download", `outdated-scans-${new Date().toISOString()}.json`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  return (
    <div className="container mx-auto px-6 py-6">
      <h2 className="text-2xl font-bold text-center mb-4 text-gray-900">Outdated Software Scan History</h2>
